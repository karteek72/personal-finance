      ...
      content: (
        <div className="flex h-full flex-col items-center justify-center space-y-3 px-6 text-center">
          ...
          <p className="text-6xl font-extrabold text-white">
            {usd(Number.parseFloat(w.totalSpent))}
          </p>
          <p className="text-base text-white/70">
            across {w.transactionCount.toLocaleString()} transactions
          </p>
          <div className="mt-2 space-y-1 text-sm text-white/80">
            <p>
              Your top category:{" "}
              <strong className="text-white">{w.topCategory.name}</strong>
            </p>
    ...
    },
    {
      ...
      content: (
        <div className="flex h-full flex-col items-center justify-center space-y-3 px-6 text-center">
          ...
          <p className="text-6xl font-extrabold text-white">
            {usd(Number.parseFloat(w.totalSaved))}
          </p>
          <p className="text-base text-white/70">
            a {formatSavingsRatePercent(w.savingsRate, 1)} savings rate
          </p>
          {w.peerPercentile ? (
    ...
    },
    {
      ...
      content: (
        <div className="flex h-full flex-col items-center justify-center space-y-3 px-6 text-center">
          ...
          <p className="text-base text-white/70">
            Experiences over things. You chose memories.
          </p>
          <div className="mt-2 space-y-1 text-sm text-white/80">
        ...
        <div className="flex h-full flex-col items-center justify-center space-y-3 px-6 text-center">
          ...
          <div className="w-full space-y-2 text-left">
            {w.moments.map((m) => (
              <div
                key={m.label}
                className="flex justify-between rounded-xl bg-white/20 px-4 py-2.5"
              >
            ...
            {w.goals.map((g) => (
              <div key={g.label} className="rounded-xl bg-white/20 px-4 py-2.5">
                <div className="mb-1 flex justify-between text-xs text-white">
                  <span>{g.label}</span>
                  <span>
                    {g.pct}% → {usd(Number.parseFloat(g.target))}
                  </span>
                </div>
...
/** Full-screen year-in-review overlay; opened from notifications. */
export function WrappedOverlay() {
  const [slide, setSlide] = useState(0);
  const open = useWrappedStore((state) => state.open);
  const closeWrapped = useWrappedStore((state) => state.closeWrapped);
  const { hasAccounts } = useHasActiveAccounts();
  const { data, isError } = useWrapped();

  if (!open || !hasAccounts || isError || !data || data.transactionCount === 0) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 p-4">
      ...
      <div
        className="relative mx-auto w-full max-w-sm overflow-hidden rounded-[var(--radius-xl)]"
        style={{
          aspectRatio: "9/16",
      ...
      >
        <div className="absolute left-4 right-4 top-4 z-10 flex gap-1">
          {slides.map((_, i) => (
            <div
              key={i}
              className="h-0.5 flex-1 cursor-pointer overflow-hidden rounded-full bg-white/40"
              onClick={() => setSlide(i)}
            >
              <div
      ...
      <div className="mt-4 flex items-center gap-4">
        <button
          ...
          onClick={() => slide > 0 && setSlide(slide - 1)}
          disabled={slide === 0}
          className="rounded-[var(--radius-sm)] border border-white/30 px-4 py-1.5 text-sm font-semibold text-white disabled:opacity-30"
        >
          ← Prev
        </button>
