...
const PREVIEW_BANNER = (
  <div className="mb-5 flex items-center gap-2 rounded-[var(--radius-sm)] border border-amber-400/40 bg-amber-400/10 px-4 py-2.5 text-sm text-amber-700 dark:text-amber-300">
    ...
    <span><strong>Preview</strong> — Merchant & Income analytics is a planned feature. Data shown is illustrative.</span>
  </div>
);
...
const INCOME_MONTHS = ["Dec", "Jan", "Feb", "Mar", "Apr", "May"];
const INCOME = [6520, 6520, 6520, 7100, 6520, 7340];
const SIDE_INCOME = [0, 240, 0, 420, 180, 610];
...
export default function MerchantsPage() {
  ...
  const avgIncome = totalIncome / INCOME_MONTHS.length;
  const variance = Math.round(
    (Math.sqrt(INCOME.reduce((s, v) => s + Math.pow(v - INCOME.reduce((a, b) => a + b, 0) / INCOME.length, 2), 0) / INCOME.length) /
      (INCOME.reduce((a, b) => a + b, 0) / INCOME.length)) *
      100,
  );
  const maxIncomeBar = Math.max(...INCOME.map((v, i) => v + (SIDE_INCOME[i] ?? 0)));
      ...
      {tab === "merchants" ? (
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              { label: "Top merchant", value: "Amazon", sub: money(1284.5) },
              { label: "Most visited", value: "Starbucks", sub: "47 visits" },
              { label: "Fastest growing", value: "DoorDash", sub: "+23% MoM" },
              { label: "Merchants tracked", value: "63", sub: "6 active monthly" },
            ].map((k) => (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              ...
              { label: "Sources", value: "3", color: "text-text" },
            ].map((k) => (
              <div key={k.label} className="rounded-[var(--radius-md)] border border-border bg-surface p-3">
                <p className="text-[11px] font-medium text-text-muted">{k.label}</p>
          ...
          <div className="rounded-[var(--radius-md)] border border-border bg-surface p-4">
            ...
            <p className="mt-1 text-sm text-text-muted">
              Your income is <strong>{100 - variance}% stable</strong> month-to-month. Side income grew to {money(610)} in May — at this pace it could cover a full car payment by Q4.
            </p>
          </div>
        </>
      )}
